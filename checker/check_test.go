...
func TestTriggerChecker_PrepareMetrics(t *testing.T) {
	...
	Convey("Get metrics data to check:", t, func() {
		...
		// Convey("last check has alone metrics", func() {
		// 	triggerChecker.lastCheck = &moira.CheckData{
		// 		Metrics: map[string]moira.MetricState{
		// 			"first":  {Values: map[string]float64{"t1": 0, "t2": 0}, AloneMetrics: map[string]string{"t2": "alone"}},
		// 			"second": {Values: map[string]float64{"t1": 0, "t2": 0}, AloneMetrics: map[string]string{"t2": "alone"}},
		// 			"third":  {Values: map[string]float64{"t1": 0, "t2": 0}, AloneMetrics: map[string]string{"t2": "alone"}},
		// 		}}
		// 	Convey("fetched metrics is empty", func() {
		// 		_, _, err := triggerChecker.prepareMetrics(metricSource.FetchedMetrics{})
		// So(prepared, ShouldHaveLength, 3)
		// for _, actualMetricData := range prepared {
...
func TestTriggerChecker_Check(t *testing.T) {
	...
	eventMetrics := map[string]moira.MetricState{
		metric: {
			...
			Suppressed:     false,
			Timestamp:      57,
			Values:         map[string]float64{"t1": 64},
		},
	}
