...
func (triggerChecker *TriggerChecker) prepareMetrics(fetchedMetrics metricSource.FetchedMetrics) (metricSource.TriggerMetricsToCheck, metricSource.MetricsToCheck, error) {
	...
	multiMetricTargets, aloneMetrics := preparedPatternMetrics.FilterAloneMetrics()
	if !aloneMetrics.IsUbiquitousMetrics() && len(multiMetricTargets) == 0 {
		// Scenario when trigger has only targets with different alone metrics
		// TODO(litleleprikon): return error
	}

	withCommonMetrics := multiMetricTargets.CrossIntersection() // TODO(litleleprikon): decide what to do with filtered metrics

	converted := withCommonMetrics.ConvertForCheck()
	if len(duplicates) > 0 {
...
func (triggerChecker *TriggerChecker) preparePatternMetrics(fetchedMetrics metricSource.FetchedPatternMetrics) (metricSource.TriggerPatternMetrics, []string) {
	...
	populated := deduplicated.PopulateWithEmptyNewMetrics(lastCheckMetricNames, triggerChecker.from, triggerChecker.until)
	result := metricSource.NewTriggerPatternMetrics(populated)
...
type ErrDifferentAloneMetrics struct {
	TriggerID   string
	MetricNames []string
}

func (e ErrDifferentAloneMetrics) Error() string {
	return fmt.Sprintf("trigger %s have set of alone metrics with different names: %s", e.TriggerID, strings.Join(e.MetricNames, ", "))
}
