...
// PopulateWithEmptyNewMetrics is a function that takes the list of metric names that first appeared and
// adds metrics with this names and empty values.
func (m FetchedPatternMetrics) PopulateWithEmptyNewMetrics(lastMetrics map[string]bool, from, until int64) FetchedPatternMetrics {
	result := NewFetchedPatternMetricsWithCapacity(len(m))
	for _, metric := range m {
		result = append(result, metric)
		delete(lastMetrics, metric.Name)
	}
	for metricName := range lastMetrics {
		if _, ok := lastMetrics[metricName]; ok {
			step := defaultStep
			if len(m) > 0 && m[0].StepTime != 0 {
				step = m[0].StepTime
			}
			metric := MakeEmptyMetricData(metricName, step, from, until)
			result = append(result, *metric)
		}
	}
	return result
...
// MetricsAmount is a function that returns amount of all metrics in TriggerMetricsData
func (m FetchedMetrics) MetricsAmount() int {
	result := 0
	for _, v := range m {
		result += len(v)
	}
...
func (m FetchedMetrics) AddMetrics(target int, metrics FetchedPatternMetrics) {
...
// HasAnyEmptyPatternMetrics is a function that checks that any of inner PatternMetrics is empty
func (m FetchedMetrics) HasAnyEmptyPatternMetrics() bool {
